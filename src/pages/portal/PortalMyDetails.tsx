    ...
    <div style={{ paddingTop: 24, paddingBottom: 100 }}>
      ...
      <div style={sectionCardStyle}>
        ...
        <div className="space-y-2 mb-3">
          {boats.map(b => (
            editingBoatId === b.id ? (
              <div key={b.id} style={{ background: 'var(--portal-card-bg)', border: `1px solid var(--portal-card-border)`, borderRadius: 8, padding: 12 }}>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <Button onClick={() => saveBoatEdit(b.id)} disabled={savingBoat} style={{ height: 36, background: 'var(--portal-primary)', color: '#FFFFFF', fontWeight: 500, borderRadius: 'var(--portal-button-radius)', paddingLeft: 14, paddingRight: 14 }}>
                    {savingBoat && <Loader2 className="h-4 w-4 animate-spin mr-2" />}
                    Save
                  </Button>
                  <Button onClick={cancelEditBoat} variant="outline" style={{ height: 36, borderRadius: 'var(--portal-button-radius)', paddingLeft: 14, paddingRight: 14 }}>Cancel</Button>
                </div>
              </div>
            ) : (
              <div key={b.id} className="relative" style={{ background: 'var(--portal-card-bg)', border: `1px solid var(--portal-card-border)`, borderRadius: 8, padding: 12 }}>
                ...
                <div className="absolute top-3 right-3 flex items-center gap-3">
                  <button onClick={() => startEditBoat(b)} style={{ fontSize: 13, color: 'var(--portal-accent)', background: 'none', border: 'none', cursor: 'pointer' }}>Edit</button>
                  <button onClick={() => removeBoat(b.id)} style={{ fontSize: 13, color: 'var(--portal-danger)', background: 'none', border: 'none', cursor: 'pointer' }}>Remove</button>
                </div>
              </div>
            )
          ))}
