-- Stock management is not live yet, so commit_cart_items must not refuse sales
-- when stock_level is below the requested qty. Stock is still decremented (and
-- may go negative) so movement data exists once stock management is built.
